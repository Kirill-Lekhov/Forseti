...
class Distance(BaseFunction):
	...
	def check_command(self):
		if bool(search(self.COMMAND_FORMAT, self.command)):
			...
			if len(parts) != 2:
				raise SyntaxError("Command doesn't support combined-condition")
			...
			raise SyntaxError("Unsupported command format. You must use: {word1} [|c{number} or |w{number}] {word2}")
...
if __name__ == "__main__":
	df = Distance("\sme |w2 is", "Somebody once told me the world is gonna roll me").res
	print(df)
	df = Distance("\sme |w2 -is", "Somebody once told me the world is gonna roll me").res
	print(df)
	df = Distance("\sme |c11 is", "Somebody once told me the world is gonna roll me").res
	print(df)
	df = Distance("\sme |c11 -is", "Somebody once told me the world is gonna roll me").res
	print(df)
